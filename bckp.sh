...
# AJA: 15/09/2022	Add fix FK_JAC_SIT error
#================== Mail address =========================================
...
echo "------> Fix FK_JAC_SIT error"
  ...
  DELETE_JAAS_QUERY="delete from jaas_config where site_code=0;"
  ...
  INSERT_JAAS_QUERY="insert into JAAS_CONFIG select HIBERNATE_SEQUENCE.nextval, 0, 'Console', 'fr.sgf.wit.security.login.module.GPRoleLoginModule', 'REQUIRED' from DUAL;"
  COMMIT_QUERY="commit;"

# Connect to the database and execute the queries
#  echo "${DELETE_JAAS_QUERY}" | sqlplus ${DB_USER}/${DB_PASSWORD}@${DB_SID}
#  echo "${INSERT_SITES_QUERY}" | sqlplus ${DB_USER}/${DB_PASSWORD}@${DB_SID}
#  echo "${INSERT_JAAS_QUERY}" | sqlplus ${DB_USER}/${DB_PASSWORD}@${DB_SID}
#  echo "${COMMIT_QUERY}" | sqlplus ${DB_USER}/${DB_PASSWORD}@${DB_SID}

  echo "${DELETE_JAAS_QUERY}" | sqlplus ${webintake_db_user}/${webintake_db_password}@${webintake_db_host}:${webintake_db_port}/${webintake_db_oracle_service_name}
  echo "${INSERT_SITES_QUERY}" | sqlplus ${webintake_db_user}/${webintake_db_password}@${webintake_db_host}:${webintake_db_port}/${webintake_db_oracle_service_name}
  echo "${INSERT_JAAS_QUERY}" | sqlplus ${webintake_db_user}/${webintake_db_password}@${webintake_db_host}:${webintake_db_port}/${webintake_db_oracle_service_name}
  echo "${COMMIT_QUERY}" | sqlplus ${webintake_db_user}/${webintake_db_password}@${webintake_db_host}:${webintake_db_port}/${webintake_db_oracle_service_name}
